...
## Создание модели и сохранение в БД

Для создания модели существует 2 способа:
...
В Mongoose существует 3 метода для поиска моделей в базе данных
...
```
вернет все модели где поле `name` равно `"Jon"`
...
```js
awiat User.find({role:'user'}, 'name', {sort:{dateOfBirth:1}, limit: 10, skip: 10})
```

### `Model.findOne(condition, projections, options, callback)`
Этот метод аналогичен предыдущему методу, за исключением того, что он возвращает первую найденную модель, 
которая соответствует условиям поиска

### `Model.findById(id, projection, options, callback)`
Это сокращенный метод для поиска модели по ее идентификатору. В качестве аргумента `id` можно передать строку, число или `ObjectId`

```js
await User.findById('4d5ea62fd76aa1136000000c')
```

## Изменение свойств модели

Изменить свойства модели можно двумя способами: с помощью статических метовод модели и уже имея созданный экземпляр класса.

### Изменение свойств модели с использованием статических методов

```js
await User.update(
  
  // первый параметр - критерии поиска, 
  // тут можно указать все то же самое что и в параметре `condition` метода `find` 
  {login:"Jon_Snow"}, 
  
  // второй параметр - это то что необходимо сделать с документами 
  {name:"Jonny"},
  
  // третим параметром можно указать ряд опций
  {}
 );
```

Метод `update` внесет изменения в первую найденную запись

Так же существует еще 2 метода, которые позволяют изменять записи:
* `updateMany` - изменит все записи, найденные по указанному критерию;
* `updateOne` - изменит только одну записть (в методе `update` можно указать параметр, который укажет изменить все записи)

### Изменение свойств записи через методы экземпляра класса

Если же у вас есть уже есть объек модели, в таком случае вы можете изменить поля следующим образом:

```js
const user = await User.findById('4d5ea62fd76aa1136000000c');

user.name = 'Jon';
```

после изменения модели надо не забыть ее сохранить используя метод `save`

### Pre- post- хуки

Mongoose позволяет добавлять дополнительное поведение при определенных событиях, таких как изменение записи, сохранение записи, поиск и [других](https://mongoosejs.com/docs/middleware.html).

Представьте, что нам надо расширить модель `User` добавив аттрибут `password`
...
Хранить пароли в открытом виде - небезопасно, т.к. ваша база данных может быть украдена.
Обычно для того, что бы спрятать пароль используются различные функции хэширования, 
которые видоизменяют пароль без возможности его восстановления.

При этом для проверки, правильный ли пароль был введен пользователем при логине, 
введенный пароль снова хэшируется и полученный хэш сравнивается с тем, что храниться 
в базе данных.

Для того, что бы при сохранении пароля сохранить в базу данных его хэш можно воспользоваться 
хуками, которые предоставляет mongoose
...
Middleware выше при сохранении модели проверит, изменено ли поле `password`, 
и если изменено - замент его значение.
Таким образом в базе сохранится уже хэшированое значение

Вы так же можете использовать хуки после того, как что-то произошло
...
Обратите внимание, что в качестве функций в хуках используются обычные (не стрелочные) функции.
Это связано с тем, что при их вызове mongoose в качестве this передаст модель или запрос в эту функцию.
При использовании стрелочных функций это невозможно 
